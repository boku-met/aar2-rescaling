...
def main():
    ...
    for file in infiles_pr:
        ...
        def parallel_loop(y):
            cur_pr = ds_in_pr.tas[ds_in_pr.time.dt.year == y].load()
            cdg_days = xr.where(cur_pr >= 19, (cur_pr-19), 0)
            d5_mean_tmax = cdg_days.sum(dim="time", skipna=True)
            return d5_mean_tmax
        ...
        pr_annual.attrs.update({"cell_methods":"time: point within days time: sum over days"
                     "(Sum of differences between current tmean and 19°C for all days exceeding 19°C)",
                      "long_name": "Sum of differences between current daily mean temperature and 19 degC" })
        ...
        file_attrs = {'title': 'Cooling Degree Days',
         'institution': 'Institute of Meteorology and Climatology, University of '
         'Natural Resources and Life Sciences, Vienna, Austria',
         'source': modelname,
         'comment': 'Annual sum of difference between daily mean temperature and 19°C for all days exceeding 19°C',
         'Conventions': 'CF-1.8'}
        
        ds_out = xr.Dataset(data_vars={"cdg_days": pr_annual,
        ...
        if path_out.endswith("/"):
            ...
            path_out += "/"
        outf = path_out + "cooling_degree_days_" + modelname + "_annual_{0}-{1}.nc".format(min(years), max(years))
