...
for file in infiles_pr:
    ...
    def parallel_loop(y):
        ...
        pr_sum = cur_pr.resample(time="M", skipna=True).sum()
        return pr_sum
    ...
    pr_1mm.attrs.update({"cell_methods":"time: sum within days time: sum over days "
                    ...
                    "long_name": "Monthly precipitation sum" })
    ...
    file_attrs = {'title': 'Monthly Precipitation Sum',
        'institution': 'Institute of Meteorology and Climatology, University of '
        'Natural Resources and Life Sciences, Vienna, Austria',
        'source': modelname,
        'comment': 'Monthly sum of daily precipitation sums',
        'Conventions': 'CF-1.8'}
    
    ds_out = xr.Dataset(data_vars={"pr": pr_1mm,
    ...
    if path_out.endswith("/"):
        ...
        path_out += "/"
    outf = path_out + "pr_" + modelname + "_monthly_{0}-{1}.nc".format(min(years), max(years))
