...
forsite_mask = "/nas/nas5/Projects/AAR2_rescaling/aar2-rescaling/data/masks/mask_regions_AT_forsite_2.nc"
region_names = {1:"West", 2:"North",3:"South"}

seasons = ["DJF", "MAM", "JJA", "SON"]

indicator_searchterm = "extreme_precipitation_"
varname = "extreme_precipitation"
...
mask_forsite = xr.open_dataset(forsite_mask)
...
for f in infiles_gwls[1:]:
    ...
    for rn in region_names.keys():
        ...
        ars = anomalies.values.flatten()
        ars = ars[~np.isnan(ars)]
        par_results.append(ars)
            
vis_data_cm5 = [x for x in par_results]

f2 = xr.open_dataset(spart_file).sel(time=slice("2001","2020"))
vis_data_obs = []
for rn in region_names.keys():
    ...
    #area_refperiod = area_refperiod[area_refperiod.time.dt.season == season]

    #area_refperiod = area_refperiod.resample(time="A").mean(dim="time", skipna=True)
...
fig, axs = plt.subplots(figsize = (6.88/2, 6.88/2.7))
labels = "GWL1.5", "GWL2.0","GWL3.0","GWL4.0"
# set basecolours for boxes
basecolours = ["#3288bd", "#99d594", "#fee08b", "#fc8d59"]
obscolour = "#353535"
plt.axhline(0, color="gray")
axs.boxplot(vis_data_obs, positions=[1,2,3], widths=0.7,patch_artist=True,
            medianprops={"color": "white", "linewidth": 1.05},
            boxprops={"facecolor": obscolour, "edgecolor": "white",
                          "linewidth": 0.5},
            whiskerprops={"color": obscolour, "linewidth": 1},
            capprops={"color": obscolour, "linewidth": 1},
            flierprops={"markeredgecolor": obscolour, "linewidth": 1.0, "markersize" : 3})
bp = axs.boxplot(vis_data_cm5, positions=[5, 6, 7, 9, 10, 11, 13, 14, 15, 17, 18, 19], widths=0.7,patch_artist=True,
            medianprops={"color": "white", "linewidth": 1.05},
...
for it in items:
    if ("whisk" in it or "cap" in it):
        ...
        for bps, colors in zip( bp[it], groupcolors):
            plt.setp(bps, markeredgecolor=colors, markersize = 3)


#axs.tick_params(labelsize=14)
axs.set_ylabel("Change in SPEI (sd)", fontsize = 10, labelpad=12)
...
axs.yaxis.set_major_locator(MultipleLocator(1)) # set major tick spacing
...
#plt.ylim(-20, 30)
plt.title("Change in SPEI of dryest summer month (Apr-Sep)\nfor different regions and global warming levels\nrelative to GWL1.0", fontsize = 10, pad = 24)
#create legend entries
pt = []
pt.append(mpatches.Patch(color=obscolour, label="Observations"))
pt.append(mpatches.Patch(color=basecolours[0], label=labels[0]))
pt.append(mpatches.Patch(color=basecolours[2], label=labels[2]))
pt.append(mpatches.Patch(color="white", label=""))
pt.append(mpatches.Patch(color=basecolours[1], label=labels[1]))
pt.append(mpatches.Patch(color=basecolours[3], label=labels[3]))

axs.legend(handles = pt, loc = (0.05, -0.7), ncols = 2, fontsize = 9)

#axs.set(axisbelow=True)

outpath = "/nas/nas5/Projects/AAR2_rescaling/aar2-rescaling/data/plots/FD_plots/boxplot_SPEI_regions_gwls_changes.jpg"
outpath2 = "/nas/nas5/Projects/AAR2_rescaling/aar2-rescaling/data/plots/FD_plots/boxplot_SPEI_regions_gwls_changes.eps"
plt.savefig(outpath2, bbox_inches ="tight")
plt.savefig(outpath,dpi=600, bbox_inches ="tight")


######### Seasonal single plots #########

spart_file = "/sto0/data/Results/Indicators/pr_SPARTACUS_monthly_1961-2021.nc"
varname_spart = "pr"
for season in seasons:
    indicator_searchterm = "pr_{0}_CMIP5_GWL".format(season)
    varname = "pr"
    infiles_gwls = sorted(glob.glob(infiles_gwl_ind+indicator_searchterm+"*.nc"))
    ...
    for f in infiles_gwls[1:]:
        ...
        for rn in region_names.keys():
            ...
            anomalies = ((area_sample - refperiod) / refperiod) *100
            ars = anomalies.values.flatten()
            ars = ars[~np.isnan(ars)]
            par_results.append(ars)
    ...
    for rn in region_names.keys():
        ...
        mean_refperiod = area_refperiod.mean(dim="time", skipna = True)
        anomalies_refperiod = ((area_refperiod - mean_refperiod) / mean_refperiod * 100)
        anomalies_refperiod = anomalies_refperiod[~np.isnan(anomalies_refperiod)]
        vis_data_obs.append(anomalies_refperiod)
    ...
    #axs.tick_params(labelsize=14)
    axs.set_ylabel("Precipitation change (%)", fontsize = 10, labelpad=12)
    axs.set_xticklabels(["West","North","South","West","North","South","West","North","South","West","North","South","West","North","South"],
                        rotation=45, fontsize = 9)
    ...
    #plt.ylim(-30, 40)
    plt.title("Change in seasonal mean precipitation sum ({0})\nfor different regions and global warming levels\nrelative to GWL1.0".format(season), fontsize = 10, pad = 24)
    #create legend entries
    pt = []
    pt.append(mpatches.Patch(color=obscolour, label="Observations"))
    pt.append(mpatches.Patch(color=basecolours[0], label=labels[0]))
    ...
    outpath = "/nas/nas5/Projects/AAR2_rescaling/aar2-rescaling/data/plots/FD_plots/boxplot_pr_{0}_regions_gwls_changes.jpg".format(season)
    outpath2 = "/nas/nas5/Projects/AAR2_rescaling/aar2-rescaling/data/plots/FD_plots/boxplot_pr_{0}_regions_gwls_changes.eps".format(season)
    plt.savefig(outpath2, bbox_inches ="tight")
    plt.savefig(outpath,dpi=600, bbox_inches ="tight")
