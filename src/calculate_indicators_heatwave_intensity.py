...
def main():
    ...
    for file in infiles_pr:
        ...
        attr_dict = {"coordinates": "time y x", 
                     "grid_mapping": "crs", 
                     "standard_name": "mean_temperature_of_five_day_heatwaves", 
                        "units": "degC"}
        pr_annual.attrs = attr_dict

        pr_annual.attrs.update({"cell_methods":"time: mean over days time: max over years"
                     "(Maximum of mean tmax for 5-day periods)",
                      "long_name": "Maximum of the mean maximum daily temperature of five day periods" })
        ...
        file_attrs = {'title': 'Heat Wave Intensity',
         'institution': 'Institute of Meteorology and Climatology, University of '
         'Natural Resources and Life Sciences, Vienna, Austria',
         'source': modelname,
         'comment': 'Maximum of the mean daily maximum temperature of five day periods',
         'Conventions': 'CF-1.8'}
        
        ds_out = xr.Dataset(data_vars={"d5_mean_tmax": pr_annual,
        ...
        if path_out.endswith("/"):
            ...
            path_out += "/"
        outf = path_out + "heatwave_intensity_" + modelname + "_annual_{0}-{1}.nc".format(min(years), max(years))
